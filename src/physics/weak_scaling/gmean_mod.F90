...
!!XXgoldyXX: ^ debug only
   ...
   subroutine gmean_arr(arr, arr_gmean, nflds)
      ...
      ! Local variables
      !
      integer                       :: max_flds  ! Max #fields on any iteration
      integer                       :: num_flds  ! #fields on current iteration
      integer                       :: wnum_flds ! # fields for this round
      integer                       :: fld_start, fld_end
      integer                       :: wfld_start, wfld_end
      integer                       :: fld_ind, col_ind
      integer                       :: send_cols ! Total cols on send tasks
      integer                       :: recv_cols ! Total cols on receive tasks
      integer                       :: windex, wpos ! For weighted field
      integer                       :: dindex    ! Receive task index
      integer                       :: round_tsknum ! # receive tasks per round
      integer                       :: dest_nflds ! # fields recv. by each task
      integer                       :: ierr
      ...
      real(r8),         allocatable :: psum_arr(:,:) ! For partial sums
      real(r8),         allocatable :: sum_arr(:)    ! For final sum on master
      integer,          allocatable :: sum_rflds(:), sum_rdisps(:)
...
!!XXgoldyXX: v debug only
#if 1
call gmean_arr_test(arr, arr_gmean, nflds)
...
!!XXgoldyXX: ^ debug only
         ...
         allocate(send_cnts(0:npes-1), stat=ierr)
         call handle_allocate_error(ierr, subname, 'send_cnts')
         allocate(send_disps(0:npes-1), stat=ierr)
         call handle_allocate_error(ierr, subname, 'send_disps')
         allocate(recv_cnts(0:npes-1), stat=ierr)
         call handle_allocate_error(ierr, subname, 'recv_cnts')
         allocate(recv_disps(0:npes-1), stat=ierr)
         call handle_allocate_error(ierr, subname, 'recv_disps')
         max_flds = MIN(column_reorder%max_nflds, nflds)
         recv_cols = SUM(column_reorder%recv_cnts)
         round_tsknum = SIZE(column_reorder%dest_tasks) /                     &
              column_reorder%num_rounds
         do fld_start = 1, nflds, column_reorder%max_nflds
            ...
            ! dest_nflds is how many fields we expect to receive on this task
            dest_nflds = MIN(column_reorder%my_nflds, MAX(0, num_flds + 1 -   &
                 (column_reorder%strt_nfld + column_reorder%my_nflds - 1)))
            if (allocated(psum_arr)) then
               if (SIZE(psum_arr, 1) /= dest_nflds) then
                  deallocate(psum_arr)
               end if
            end if
            if (.not. allocated(psum_arr)) then
               allocate(psum_arr(dest_nflds, recv_cols), stat=ierr)
               call handle_allocate_error(ierr, subname, 'psum_arr')
            end if
            ! Retrieve the raw data and set counts and displacements.
            send_cols = 0
            send_cnts = 0
            send_disps = 0
            recv_cnts = 0
            recv_disps = 0
            wfld_start = 0
            ! Compute send counts and displacements for each sending task
            do windex = 1, column_reorder%num_rounds
               send_cols = send_cols + 1
               wnum_flds = column_reorder%num_rflds(windex)
               ! Where does this round go?
               do dindex = (windex-1)*round_tsknum, (windex*round_tsknum)-1
                  ! We are sending <nflds> from each task but distributed
                  wfld_end = MIN(wfld_start + wnum_flds - 1, nflds)
                  wnum_flds = MAX(wfld_end - wfld_start + 1, 0)
                  send_cnts(column_reorder%dest_tasks(dindex)) =              &
                       column_reorder%task_sizes(dindex) * wnum_flds
                  send_disps(dindex) =                                        &
                       column_reorder%send_disps(dindex) * wnum_flds
               end do
               wfld_start = wfld_end + 1
               if (wfld_start >= num_flds) then
                  exit
               end if
            end do
            ! Compute receive counts
            recv_cnts(:) = column_reorder%recv_cnts(:) * dest_nflds
            recv_disps(:) = column_reorder%recv_disps(:) * dest_nflds
            send_cols = columns_on_task * send_cols
            allocate(weighted_field(nflds, send_cols))
            wfld_start = fld_start
            wpos = 1
            do windex = 1, column_reorder%num_rounds
               wnum_flds = column_reorder%num_rflds(windex)
               wfld_end = MIN(wfld_start + wnum_flds - 1, fld_end)
               call weighted_field_p(arr(:,:,wfld_start:wfld_end),            &
                    weighted_field(:, wpos:wpos+columns_on_task-1),           &
                    column_reorder%send_reorder, flds_first=.true.)
               wpos = wpos + columns_on_task
               wfld_start = wfld_end + 1
               if (wfld_start >= num_flds) then
                  exit
               end if
         ...
         if (column_reorder%mpi_comm /= MPI_COMM_NULL) then
            if (iam == masterprocid) then
               dindex = SIZE(column_reorder%dest_tasks)
            else
               dindex = 0
            end if
            allocate(sum_arr(dindex), stat=ierr)
            call handle_allocate_error(ierr, subname, 'sum_arr')
            sum_arr = 0
            allocate(sum_rflds(dindex))
            call handle_allocate_error(ierr, subname, 'sum_rflds')
            sum_rflds = 0
            allocate(sum_rdisps(size(sum_rflds)))
            call handle_allocate_error(ierr, subname, 'sum_rdisps')
            ! Calculate receive counts and displacements
            ! Recieve counts are found by gathering all the send counts
            call MPI_gather(dest_nflds, 1, MPI_INTEGER, sum_rflds, 1,         &
                 MPI_INTEGER, column_reorder%recv_master_id,                  &
                 column_reorder%mpi_comm, ierr)
            if (column_reorder%recv_master_id == column_reorder%recv_iam) then
               sum_rdisps = 0
               do dindex = 2, SIZE(column_reorder%dest_tasks)
                  if (sum_rflds(dindex) > 0) then
                     sum_rdisps(dindex) = sum_rdisps(dindex - 1) +            &
                          sum_rflds(dindex - 1)
                  end if
               end do
            end if
            sum_arr = 0.0_r8
            call MPI_gatherv(wsum(1:dest_nflds), dest_nflds, MPI_REAL8,       &
                 sum_arr, sum_rflds, sum_rdisps, MPI_REAL8,                   &
                 column_reorder%recv_master_id, column_reorder%mpi_comm, ierr)
            deallocate(sum_rflds)
            deallocate(sum_rdisps)
            if (column_reorder%recv_master_id == column_reorder%recv_iam) then
               wfld_end = 0
               windex = 1
               col_ind = 1
               do windex = 0, SIZE(column_reorder%dest_tasks) - 1
                  if (MOD(windex, round_tsknum) == 0) then
                     wfld_start = wfld_end + 1
                     dindex = (windex / round_tsknum) + 1
                     wnum_flds = column_reorder%num_rflds(dindex)
                     wfld_end = MIN(nflds, wfld_start + wnum_flds - 1)
                     if (wfld_start > nflds) then
                        exit
                     end if
                  end if
                  do dindex = wfld_start, wfld_end
                     arr_gmean(dindex) = arr_gmean(dindex) + sum_arr(col_ind)
                     col_ind = col_ind + 1
                     if (col_ind > SIZE(column_reorder%dest_tasks)) then
                        exit
                     end if
                  end do
               end do
            end if
            if (allocated(sum_arr)) then
               deallocate(sum_arr)
            end if
         end if
...
!!XXgoldyXX: v debug only
   ...
   subroutine gmean_arr_test (arr, arr_gmean, nflds)
