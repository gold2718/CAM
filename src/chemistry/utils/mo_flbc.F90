module mo_flbc
   ...
   interface flbc_set
      module procedure flbc_set_all
      module procedure flbc_set_one
   end interface flbc_set
...
contains
   ...
   subroutine flbc_inti(flbc_file, flbc_list, flbc_timing_in, co2vmr, ch4vmr, n2ovmr, f11vmr, f12vmr)
      ...
      do m = 1,max_nflbc
         ...
         else if(trim(flbcs(flbc_cnt)%species) == 'CFC12') then
      ...
      if(astat/= 0) then
         call endrun('flbc_inti: failed to allocate dates array; error = '//int2str(astat))
      end if
      allocate(times(ntimes),stat=astat)
      if(astat/= 0) then
         write(iulog,*)
         call endrun('flbc_inti: failed to allocate times array; error = '//int2str(astat))
      end if
      ...
      if(time_type /= 'CYCLICAL') then
         if(wrk_time < times(1) .or. wrk_time > times(ntimes)) then
            call endrun('flbc_inti: time out of bounds for dataset = '//trim(filename))
         end if
         do n = 2,ntimes
      ...
      case('CYCLICAL')
         ...
         if((tim_ndx(2) - tim_ndx(1)) < 2) then
            call endrun('flbc_inti: cyclical lb conds require at least two time points')
         end if
      case('SERIAL')
      ...
      do m = 1,flbc_cnt
         ...
         if(astat/= 0) then
            call endrun('flbc_inti: failed to allocate lbc vmr; error = '//int2str(astat))
         end if
         flbcs(m)%has_mean = file_has_gmean(ncid,flbcs(m)%species)
         if (flbcs(m)%has_mean) then
            allocate(flbcs(m)%vmr_mean(t1:t2),stat=astat)
            if(astat/= 0) then
               call endrun('flbc_inti: failed to allocate lbc vmr_mean; error = '//int2str(astat))
            end if
         end if
         !-----------------------------------------------------------------------
         ! 	... readin the flbc vmr
         ...
         if(wrk_time > times(tim_ndx(2))) then
            ...
            do m = 1,flbc_cnt
               if(associated(flbcs(m)%vmr)) then
                  ...
                  if(astat/= 0) then
                     call endrun('flbc_chk: failed to deallocate flbc vmr; error = '//int2str(astat))
                  end if
               end if
               allocate(flbcs(m)%vmr(pcols,begchunk:endchunk,t1:t2),stat=astat)
               if(astat/= 0) then
                  call endrun('flbc_chk: failed to allocate flbc vmr; error = '//int2str(astat))
               end if
...
!!$          end if
   ...
   subroutine flbc_get(ncid, lbcs, initial, read_gmean)
      ...
      if(ierr /= 0) then
         ...
         call endrun('srf_emis_get: locl_emis allocation error = '//int2str(ierr))
      end if
      ...
      initialization : if(initial) then
         ...
         if(ierr /= 0) then
            call endrun('flbc_get: lat allocation error = '//int2str(ierr))
         end if
         ...
         if (ierr == PIO_NOERR) then
            ...
            if(ierr /= 0) then
               call endrun('flbc_get: lon allocation error = '//int2str(ierr))
            end if
      ...
      if (ndims==2) then
         ...
         if(ierr /= 0) then
            call endrun('flbc_get: wrk_zonal allocation error = '//int2str(ierr))
         end if
      else
         allocate(wrk(nlon,nlat,tcnt), stat=ierr)
         if(ierr /= 0) then
            call endrun('flbc_get: wrk allocation error = '//int2str(ierr))
         end if
      end if
      ...
      if (ndims==2) then
         ...
         if(ierr /= 0) then
            call endrun('flbc_get: Failed to deallocate wrk_zonal, ierr = '//int2str(ierr))
         end if
      else
         deallocate(wrk, stat=ierr)
         if(ierr /= 0) then
            call endrun('flbc_get: Failed to deallocate wrk, ierr = '//int2str(ierr))
         end if
      end if
      if (read_gmean) then
      ...
      if(ierr /= 0) then
         call endrun('flbc_get: Failed to deallocate locl_vmr; ierr = '//int2str(ierr))
      end if

   end subroutine flbc_get

   subroutine flbc_set_all(vmr, ncol, lchnk, map)
      !--------------------------------------------------------
      !	... set the lower bndy values for all FLBC species
      !--------------------------------------------------------
      ...
      do mind = 1, flbc_cnt
         if (flbcs(mind)%spc_ndx > 0) then
            ncnst = map(flbcs(mind)%spc_ndx)
            ! If the GHG happens to be an advected specie, but not a chemical
            ! specie (e.g., CO2 when the carbon cycle is on in standard CAM),
            ! then ncnst=0 and we need to skip setting the LBC.
            if (ncnst > 0) then
               vmr(:ncol,pver,ncnst) = flbcs(mind)%vmr(:ncol,lchnk,last)  &
   ...
   end subroutine flbc_set_all

   subroutine flbc_set_one(specie_name, vmr, lchnk, ncol, vmr_set)
      !--------------------------------------------------------
      !	... set the lower bndy values for one FLBC specie
      !--------------------------------------------------------
      ...
      real(r8),          intent(inout) :: vmr(:) ! lower bndy conc. (mol/mol)
      integer,           intent(in)    :: lchnk
      integer,           intent(in)    :: ncol
      logical, optional, intent(out)   :: vmr_set
   ...
   subroutine get_dels(dels, last, next)
      ...
      if(flbc_timing%type /= 'CYCLICAL') then
         ...
         if(n > ntimes) then
            call endrun('flbc_set: interp time is out of bounds')
         end if
   ...
   subroutine flbc_gmean_vmr(co2vmr,ch4vmr,n2ovmr,f11vmr,f12vmr)
      ...
      else if (ghg_indices(f11eq_ndx) > 0) then
         f11vmr = global_mean_vmr(flbcs(ghg_indices(f11eq_ndx)), dels, last, next)
      end if
      if (ghg_indices(f12_ndx) > 0) then
